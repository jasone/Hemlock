...
module type SPoly = sig
  ...
  (** [empty cmper] creates an empty set associated with [cmper], which is a first-class module
      encapsulating a comparator. *)

  val singleton: ('a, 'cmp) cmper -> 'a -> ('a, 'cmp) t
  (** [singleton cmper elm] creates a set associated with [cmper] that contains [elm]. *)

  val of_list: ('a, 'cmp) cmper -> 'a list -> ('a, 'cmp) t
  (** [of_list cmper elms] creates a set associated with [cmper] that contains [elms]. *)
  ...
  *)
  ...
  (** [mem a t] returns [true] if [a] is a member of [t]; [false] otherwise. O(lg n) time complexity
  ...
  (** [insert elm t] returns a set that is equivalent to the union of a singleton set containing
      [elm] with [t]. O(lg n) time complexity if ordered, O(1) time complexity if unordered. *)

  val remove: 'a -> ('a, 'cmp) t -> ('a, 'cmp) t
  (** [remove a t] returns a set that is equivalent to the difference of [t] relative to the
      singleton set containing [a]. O(lg n) time complexity if ordered, O(1) time complexity if
      unordered. *)
  ...
  (** [union t0 t1] creates a set that is the union of [t0] and [t1]; that is, a set that contains
      all elements in [t0] or [t1]. O(m lg (n/m + 1)) time complexity if ordered, where m and n are
      the input set lengths and m <= n; Θ(m+n) time complexity if unordered. *)

  val inter: ('a, 'cmp) t -> ('a, 'cmp) t -> ('a, 'cmp) t
  (** [inter t0 t1] creates a set that is the intersection of [t0] and [t1]; that is, a set that
      contains all elements present in both [t0] and [t1]. O(m lg (n/m + 1)) time complexity if
      ordered, where m and n are the input set lengths and m <= n; Θ(m+n) time complexity if
      unordered. *)

  val diff: ('a, 'cmp) t -> ('a, 'cmp) t -> ('a, 'cmp) t
  (** [diff t0 t1] creates a set that is the difference of t0 relative to t1; that is, a set that
      contains all elements present in [t0] but not present in [t1]. O(m lg (n/m + 1)) time
      complexity if ordered, where m and n are the input set lengths and m <= n; Θ(m+n) time
  ...
  val partition_tf: f:('a -> bool) -> ('a, 'cmp) t -> ('a, 'cmp) t * ('a, 'cmp) t
  (** [partition_tf ~f t] partitions [t] into two sets for which [~f] returns [true] vs [false]. *)
