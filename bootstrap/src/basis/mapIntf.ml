...
(** Map. Note that O(1) time complexity for various unordered map operations assumes a
...
module type S = sig
  ...
  val hash_fold: ('v -> Hash.State.t -> Hash.State.t) -> ('k, 'v, 'cmp) t -> Hash.State.t
    -> Hash.State.t
  (** [hash_fold hash_fold_v t state] incorporates the hash of [t] into [state] and returns the
      resulting state. Mapping are stably hash-folded into the resulting state via the type's
      comparator [hash_fold] function for keys, and [hash_fold_v] for values. *)

  val cmper_m: ('k, 'v, 'cmp) t -> ('k, 'cmp) cmper
  (** [cmper_m t] returns a first class module that can be used to build other sets or maps with
  ...
  (** [empty cmper] creates an empty map associated with [cmper], which is a first-class module
      encapsulating a comparator. *)

  val singleton: ('k, 'cmp) cmper -> k:'k -> v:'v -> ('k, 'v, 'cmp) t
  (** [singleton cmper ~k ~v] creates a map associated with [cmper] that maps [k] to [v]. *)

  val of_alist: ('k, 'cmp) cmper -> ('k * 'v) list -> ('k, 'v, 'cmp) t
  (** [of_alist cmper kvpairs] creates a map associated with [cmper] that contains the key-value
      mappings represented by [kvpairs]. Halts on duplicate keys. *)
  ...
  (** [is_empty t] returns [true] if [t] has no mappings; [false] otherwise. O(1) time complexity.
  *)

  (** {1 Mapping operations} *)

  val mem: 'k -> ('k, 'v, 'cmp) t -> bool
  (** [mem k t] returns [true] if [k] is a key in [t]; [false] otherwise. O(lg n) time complexity if
      ordered, O(1) time complexity if unordered. *)

  val get: 'k -> ('k, 'v, 'cmp) t -> 'v option
  (** [get k t] returns the value [Some v] associated with [k] if [k] is a key in [t], [None]
      otherwise. O(lg n) time complexity if ordered, O(1) time complexity if unordered. *)

  val get_hlt: 'k -> ('k, 'v, 'cmp) t -> 'v
  (** [get_hlt k t] returns the value [v] associated with [k] if [k] is a key in [t], halts
      otherwise. O(lg n) time complexity if ordered, O(1) time complexity if unordered. *)

  val choose: ('k, 'v, 'cmp) t -> ('k * 'v) option
  (** [choose t] returns an arbitrary key-value mapping in [t] if the map is non-empty, [None]
      otherwise. *)

  val choose_hlt: ('k, 'v, 'cmp) t -> ('k * 'v)
  (** [choose_hlt t] returns an arbitrary key-value mapping in [t] if the map is non-empty, halts
      otherwise. *)

  val insert: k:'k -> v:'v -> ('k, 'v, 'cmp) t -> ('k, 'v, 'cmp) t
  (** [insert ~k ~v t] returns an incremental derivative of [t] with [k] bound to [v] if [k] is not
      a key in [t], [t] otherwise. O(lg n) time complexity if ordered, O(1) time complexity if
      unordered. *)

  val insert_hlt: k:'k -> v:'v -> ('k, 'v, 'cmp) t -> ('k, 'v, 'cmp) t
  (** [insert_hlt ~k ~v t] returns an incremental derivative of [t] with [k] bound to [v] if [k] is
      not a key in [t], halts otherwise. O(lg n) time complexity if ordered, O(1) time complexity if
      unordered. *)

  val upsert: k:'k -> v:'v -> ('k, 'v, 'cmp) t -> ('k, 'v, 'cmp) t
  (** [upsert ~k ~v t] returns an incremental derivative of [t] with [k] bound to [v], regardless of
      whether [k] is a key in [t]. O(lg n) time complexity if ordered, O(1) time complexity if
      unordered. *)

  val update: k:'k -> v:'v -> ('k, 'v, 'cmp) t -> ('k, 'v, 'cmp) t
  (** [update ~k ~v t] returns an incremental derivative of [t] with [k] bound to [v] if [k] is a
      key in [t], [t] otherwise. O(lg n) time complexity if ordered, O(1) time complexity if
      unordered. *)

  val update_hlt: k:'k -> v:'v -> ('k, 'v, 'cmp) t -> ('k, 'v, 'cmp) t
  (** [update_hlt ~k ~v t] returns an incremental derivative of [t] with [k] bound to [v] if [k] is
      a key in [t], halts otherwise. O(lg n) time complexity if ordered, O(1) time complexity if
      unordered. *)

  val remove: 'k -> ('k, 'v, 'cmp) t -> ('k, 'v, 'cmp) t
  (** [remove k t] returns an incremental derivative of [t] with no binding for [k] if [k] is a key
      in [t], [t] otherwise. O(lg n) time complexity if ordered, O(1) time complexity if unordered.
  *)

  val remove_hlt: 'k -> ('k, 'v, 'cmp) t -> ('k, 'v, 'cmp) t
  (** [remove_hlt k t] returns an incremental derivative of [t] with no binding for [k] if [k] is a
      key in [t], halts otherwise. O(lg n) time complexity if ordered, O(1) time complexity if
      unordered. *)
  ...
  val fold_until: init:'accum -> f:('accum -> ('k * 'v) -> 'accum * bool) -> ('k, 'v, 'cmp) t
    -> 'accum
  ...
  val fold: init:'accum -> f:('accum -> ('k * 'v) -> 'accum) -> ('k, 'v, 'cmp) t -> 'accum
  ...
  (** [find ~f t] iterates from left to right if ordered, or arbitrarily if unordered, over [t] and
      returns [Some (k, v)] for the first mapping which [f] returns [true], or [None] if [f] always
      returns [false]. *)
  ...
  val filter_map: f:(('k * 'v) -> 'v2 option) -> ('k, 'v, 'cmp) t -> ('k, 'v2, 'cmp) t
  (** [filter_map ~f t] creates a map with contents filtered and mapped by [~f]. Only mappings for
      which the filter-map function returns [Some v2] are incorporated into the result. Θ(n) time
      complexity. *)

  val partition_tf: f:(('k * 'v) -> bool) -> ('k, 'v, 'cmp) t -> ('k, 'v, 'cmp) t * ('k, 'v, 'cmp) t
  (** [partition_tf ~f t] partitions [t] into two maps for which [~f] returns [true] vs [false].
      Θ(n) time complexity. *)

  val partition_map: f:(('k * 'v) -> ('v2, 'v3) Either.t) -> ('k, 'v, 'cmp) t
    -> ('k, 'v2, 'cmp) t * ('k, 'v3, 'cmp) t
  (** [partition_map ~f t] partitions [t] into two maps for which [~f] returns [First v2] vs [Second
      v3]. Θ(n) time complexity. *)

  val kreduce: f:('k -> 'k -> 'k) -> ('k, 'v, 'cmp) t -> 'k option
  (** [kreduce ~f t] reduces [t] to a single key, or [None] if the map is empty. The reduction
  ...
  (** [kreduce_hlt ~f t] reduces [t] to a single key, or halts if the map is empty. The reduction
      function is assumed to be associative and commutative; thus reduction order is unspecified. *)

  val reduce: f:('v -> 'v -> 'v) -> ('k, 'v, 'cmp) t -> 'v option
  (** [reduce ~f t] reduces [t] to a single value, or [None] if the map is empty. The reduction
      function is assumed to be associative and commutative; thus reduction order is unspecified. *)

  val reduce_hlt: f:('v -> 'v -> 'v) -> ('k, 'v, 'cmp) t -> 'v
  (** [reduce_hlt ~f t] reduces [t] to a single value, or halts if the map is empty. The reduction
      function is assumed to be associative and commutative; thus reduction order is unspecified. *)
...
module type SOrd = sig
  ...
  (** [search k t] returns [(Some index)] if [k] is a key in [t]; [None] otherwise. O(lg n) time
      complexity if ordered, O(1) time complexity if unordered. *)

  val nsearch: 'k -> ('k, 'v, 'cmp) t -> (Cmp.t * uns) option
  (** [nsearch k t] searches for [k] in [t], and falls back to the nearest present succesor of [k]
  ...
  (** [cmp vcmp t0 t1] compares [t0] and [t1]. O(m+n) time complexity, where m and n are the input
      map lengths. *)

  val split: 'k -> ('k, 'v, 'cmp) t -> ('k, 'v, 'cmp) t * ('k * 'v) option * ('k, 'v, 'cmp) t
  (** [split k t] tripartitions [t] into mappings with keys \{<,=,>\} [k], respectively. *)

  (** {1 Folding, mapping, and filtering} *)

  val fold_right_until: init:'accum -> f:('accum -> ('k * 'v) -> 'accum * bool) -> ('k, 'v, 'cmp) t
    -> 'accum
  ...
  val foldi_until: init:'accum -> f:(uns -> 'accum -> ('k * 'v) -> 'accum * bool)
    -> ('k, 'v, 'cmp) t -> 'accum
  ...
  val fold_right: init:'accum -> f:('accum -> ('k * 'v) -> 'accum) -> ('k, 'v, 'cmp) t -> 'accum
  (** [fold_right ~init ~f t] folds [t] from left to right, using [init] as the initial accumulator
      value. *)

  val foldi: init:'accum -> f:(uns -> 'accum -> ('k * 'v) -> 'accum) -> ('k, 'v, 'cmp) t -> 'accum
  ...
  val findi: f:(uns -> ('k * 'v) -> bool) -> ('k, 'v, 'cmp) t -> ('k * 'v) option
  (** [findi ~f t] iterates from left to right over [t] with index provided to [f] and returns [Some
      (k, v)] for a mapping which [f] returns [true], or [None] if [f] always returns [false]. *)
  ...
  (** [filteri ~f t] creates a map with contents filtered by [~f]. Only mappings for which the
      filter function returns [true] are incorporated into the result. Θ(n) time complexity. *)

  val filteri_map: f:(uns -> ('k * 'v) -> 'v2 option) -> ('k, 'v, 'cmp) t -> ('k, 'v2, 'cmp) t
  (** [filteri_map ~f t] creates a map with contents filtered and mapped by [~f]. Only mappings for
      which the filter-map function returns [Some v2] are incorporated into the result. Θ(n) time
      complexity. *)

  val partitioni_tf: f:(uns -> ('k * 'v) -> bool) -> ('k, 'v, 'cmp) t
    -> ('k, 'v, 'cmp) t * ('k, 'v, 'cmp) t
  (** [partitioni_tf ~f t] partitions [t] into two maps for which [~f] returns [true] vs [false].
      Θ(n) time complexity. *)

  val partitioni_map: f:(uns -> ('k * 'v) -> ('v2, 'v3) Either.t) -> ('k, 'v, 'cmp) t
    -> ('k, 'v2, 'cmp) t * ('k, 'v3, 'cmp) t
  (** [partitioni_map ~f t] partitions [t] into two maps for which [~f] returns [First v2] vs
      [Second v3]. Θ(n) time complexity. *)

  val min_elm: cmp:(('k * 'v) -> ('k * 'v) -> Cmp.t) -> ('k, 'v, 'cmp) t -> ('k * 'v) option
  (** [min_elm ~f t] iterates from left to right over [t] and returns [Some (k, v)] for the leftmost
      element which always compares as [Cmp.Lt] or [Cmp.Eq], or [None] if [t] is empty. *)

  val max_elm: cmp:(('k * 'v) -> ('k * 'v) -> Cmp.t) -> ('k, 'v, 'cmp) t -> ('k * 'v) option
  (** [max_elm ~f t] iterates from left to right over [t] and returns [Some (k, v)] for the leftmost
